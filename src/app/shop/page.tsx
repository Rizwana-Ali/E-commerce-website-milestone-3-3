...
export default function Shop() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      const displayedProducts = productsData.slice(12, 30);
        ...
        <div className="grid gap-6" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))" }}>
          {products.map((product:any) => (
            <div key={product._id} className="border border-gray-300 rounded-lg p-4 bg-gray-50 text-center">
              ...
              <button onClick={() => handleAddToCart(product)} className="mt-3 bg-green-500 text-white px-4 py-2 rounded hover:bg-pink-600"><FaShoppingCart /></button>
            </div>
          ))}
        </div>
      </div>
